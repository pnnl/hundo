...
rule quality_filter_reads:
    ...
    params:
        endref = "lref={} rref={}".format(config["quality_control"].get("adapters", "") if config["quality_control"].get("adapters", "")) else "",
        fref = "fref={}".format(config["quality_control"].get("contaminants", "")) if config["quality_control"].get("contaminants", "") else "",
        mink = config["quality_control"].get("mink", 8),
        quality = config["quality_control"].get("minimum_base_quality", 10),
        hdist = config["quality_control"].get("allowable_kmer_mismatches", 1),
        k = config["quality_control"].get("reference_kmer_match_length", 31),
        qtrim = "rl",
        minlength = config["quality_control"].get("minimum_passing_read_length", 51)
    threads: config.get("threads", 1)
    shell: """bbduk2.sh -Xmx8g in={input.r1} in2={input.r2} out={output.r1} out2={output.r2} \
                  {params.endref} {params.fref} mink={params.mink} \
...
if config.get("program", "vsearch") == "vsearch":
    rule merge_reads:
        input:
        ...
        params:
            minimum_merge_length = config["quality_control"].get("minimum_merge_length", 100)
        log:
            "results/{eid}/{pid}/logs/fastq_mergepairs.log".format(eid=config["eid"], pid=CLUSTER_THRESHOLD)
        shell: """vsearch -fastq_mergepairs {input.r1} -reverse {input.r2} \
    ...
    rule fastq_filter:
        ...
        output: "results/{eid}/merged_EE%s.fasta" % str(config["quality_control"].get("maximum_expected_error", 1))
        version: VSEARCH_VERSION
        message: "Filtering FASTQ with VSEARCH with an expected maximum error rate of {params.maxee}"
        params: maxee = config["quality_control"].get("maximum_expected_error", 1)
    ...
    if config["chimera_checking"].get("uchime_denovo_prefilter", False):
        rule optional_chimera_prefilter:
            ...
            version: VSEARCH_VERSION
            message: "Chimera checking using UCHIME de novo as implemented in VSEARCH"
    ...
    rule cluster_sequences:
        ...
        version: VSEARCH_VERSION
        message: "Clustering sequences with VSEARCH where OTUs have a minimum size of {params.minsize} and where the maximum difference between an OTU member sequence and the representative sequence of that OTU is {params.otu_id_pct}%"
        params:
            minsize = config["clustering"].get("minimum_sequence_abundance", 2),
            otu_id_pct = (100 - float(config["clustering"].get("percent_of_allowable_difference", 3))) / 100.
    ...
    # if config["chimera_checking"].get("chimera_filter_seed_sequences", False):
        # implement vsearch --uchime_ref here...
    # else:
        # rename
        # cp "OTU_unfiltered.fasta" "OTU.fasta"
    ...
    rule merge_reads:
        ...
        params: minimum_merge_length = config["quality_control"].get("minimum_merge_length", 100)
    ...
    rule dereplicate_sequences:
        ...
        threads: config.get("threads", 1)
        log: "results/{eid}/{pid}/logs/uniques.log".format(eid=config["eid"], pid=CLUSTER_THRESHOLD)
        shell: """usearch -fastx_uniques {input} -fastaout {output} -sizeout \
    ...
    rule cluster_sequences:
        ...
        params:
            minsize = config["clustering"].get("minimum_sequence_abundance", 2),
            otu_radius_pct = config["clustering"].get("percent_of_allowable_difference", 3)
        log: "results/{eid}/{pid}/logs/cluster_sequences.log"
        shell: """usearch -cluster_otus {input} -minsize {params.minsize} -otus {output} \
                      -relabel OTU_ -otu_radius_pct {params.otu_radius_pct} -log {log}"""
    ...
    if config["chimera_checking"].get("chimera_filter_seed_sequences", False):
        rule remove_chimeric_otus:
            ...
            params: mode = config["chimera_checking"].get("usearch_chimera_mode", "high_confidence")
            threads: config.get("threads", 1)
    ...
    if config.get("annotation_method") == "utax":
        rule utax:
            input:
            ...
            params: utax_cutoff = config["taxonomy"].get("prediction_confidence_cutoff", 0.8)
            threads: config.get("threads", 1)
        ...
        rule fix_utax_taxonomy:
            ...
            params: kingdom = config.get("kingdom", "?")
...
if config.get("program", "vsearch") == "vsearch" or config.get("annotation_method", "blast") == "blast":
    rule blast:
        input:
            fasta = "results/{eid}/{pid}/OTU.fasta",
            db = rules.make_blast_db.output
        output: "results/{eid}/{pid}/blast/blast_hits.txt"
        params: db = config["blast_database"]["fasta"]
        threads: config.get("threads", 1)
    ...
    rule lca:
        ...
        params:
            ...
            P = config["taxonomy"].get("lca_cutoffs", "99,97,95,90,85,80,0"),
            L = config["taxonomy"].get("prediction_confidence_cutoff", 0.8)
