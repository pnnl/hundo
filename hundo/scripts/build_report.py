...
SCRIPT = """
    ...
    window.onresize = function() {
        Plotly.Plots.resize('countSummaryPlot')
        Plotly.Plots.resize('qualityScorePlot')
        Plotly.Plots.resize('taxonomyPlot')
        Plotly.Plots.resize('eePlot')
    }
    </script>
    """
...
def build_summary_table(raw, biom_df, div_df, omitted=None):
    ...
    return df_str, make_div(fig, div_id="countSummaryPlot")
...
def build_quality_plot(r1_quals):
    ...
    return make_div(fig, div_id="qualityScorePlot")
...
def build_taxonomy_plot(txt, value_cols, height=900):
    ...
    return make_div(fig, div_id="taxonomyPlot")
...
def build_ee_plot(eeplot):
    ...
    fig = go.Figure(data=data, layout=layout)
    return make_div(fig, div_id="eePlot")
