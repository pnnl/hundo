...
class VsearchHits(object):
    ...
    def __repr__(self):
        return "{cls}[{tax}]".format(cls=self.__class__.__name__, tax=self.names) #this makes it human readable if you just call teh class it will tell you what it is
    ...
    def add(self, name, percent_id):
        ...
        if self.top_fraction and self.percent_ids:
            ...
            if percent_id < (self.percent_ids[-1] * self.top_fraction): #if the last item in the bitscores * the fraction decided is less than
            ...
            # new best
                ...
                while score < percent_id * self.top_fraction: #as long as the score is not larger than the bitscore * the fraction determined
                    #then the following commands will remove the first entry in that desque object
                    self.names.popleft()
                    self.percent_ids.popleft()
                    #self.bitscores.popleft()
                    score = self.percent_ids[0] #once you have removed that first entry, set score to be equal to the "new"
        ...
        if percent_id:
            ...
            if len(self.names) > self.max_hits: #if there are more names than there are allowed maximum hits, remove the
    ...
    def majority(self):
        ...
        """
        # no repeated names
        if len(self.names) == len(set(self.names)): #if the length of names is equal to the number of unique names in
            #the list, then return the best hit
            return self.best_hit()
        else:

            # count each taxonomy, grab top taxonomy
            most_common = Counter(self.names).most_common(1)[0][0] #so of the names, take the most common,
            ...
            # left most index match
            idx = names_reversed.index(most_common) #grab the position of the most common when the order is reversed
...
def parse_vsearchhits(file, min_pid=.85, top_fraction=0.98):
    ...
    #print(toks)#so for each value in the blasthits object,
        ...
        if (float(toks["pident"]) / 100) < min_pid: continue #if the bitscore is lower than the min socore we set of 155, then
